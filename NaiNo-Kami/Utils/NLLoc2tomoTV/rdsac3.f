	...
	character*80 evstr
	character*14 uid
	character*4 staname, station(NSTAMAX)
	character*80 evlist, sacdir, h71dir, datdir, hdrdir
	real signal(NPTMAX2)
	logical staOK, h71exist, stnexist

	call read_fname(evlist, sacdir, h71dir, datdir, hdrdir)
	call read_stations(station, nsta)
	open(10, file = evlist,
     1           status = "old")
...
10	continue
	   read(10, *, end = 20) uid
	   write(6,*) uid
	   evstr = sacdir(1:length(sacdir))//'/'//uid
	   call rd_wr_hdr0(h71dir, hdrdir, datdir, 
     1                     uid, t0, h71exist)
	   if (h71exist) then
	      istn = 0
	      do ista = 1, nsta
...
c----------------------------------------------------------------------
	subroutine read_fname(evlist, sacdir, h71dir, datdir, hdrdir)

	character*(*) evlist, sacdir, h71dir, datdir, hdrdir

	write(6,*) 'Please enter event list file name : '
	read(5, '(a)') evlist
	write(6,*) 'Please enter the sac file directory name : '
	read(5, '(a)') sacdir
	write(6,*) 'Please enter the h71 file directory name : '
	read(5, '(a)') h71dir
	write(6,*) 'Please enter the hdr file directory name : '
	read(5, '(a)') hdrdir
	write(6,*) 'Please enter the dat file directory name : '
	read(5, '(a)') datdir
...
c----------------------------------------------------------------------
	subroutine read_stations(station, nsta)

	character*(*) station(*)

	open(10, file = 'OVPF_STNS1', status = 'old')
	i = 1
10	continue
	   read(10, '(a4)', end = 20) station(i)
	   i = i + 1
	go to 10
20	continue
	nsta = i - 1
...
c----------------------------------------------------------------------
	subroutine rd_wr_hdr0(h71dir, hdrdir, datdir, 
     1                        uid, t0, h71exist)
	parameter (SRATE = 0.01, IOFFSET = 100)
	
	character*(*) h71dir, hdrdir, datdir, uid
	character*35 ch
	character*80 fname
	logical h71exist

	fname = h71dir(1:length(h71dir))//'/'//uid//'.h71'
	write(6,*) fname
	...
	open(40, file = datdir(1:length(datdir))//'/'//uid//'.dat',
     1           status = 'unknown',
     2           form = 'unformatted',
     3           access = 'direct',
     4           recl = 512)
	open(80, file = hdrdir(1:length(hdrdir))//'/'//uid//'.hdr',
     1           status = 'unknown')
...
c On considère que les trois composantes ont les mêmes temps d arrivées et les mêmes paramètres de rai! Permet de récupérer des signaux (E,N) non pointés.
	do while (stn(1:3) .ne. sta(1:3) .and. (stn(1:3) .ne. '   '))
	   read(70, '(1x,a4,1x,f5.1,2(1x,i3),11x,f5.2,1x,f5.2)', 
     1                end = 10) 
     1                stn, dist, iazm, iain, tarr, tp
	end do
	if (stn(1:3) .eq. sta(1:3)) then
	   if (tp .lt. 0.) tp = tp + 60.
	   write(80, '(a4,1x,f6.3,1x,f5.2,2(1x,i3),2x,i1)')
     1                 sta, tp, dist, iazm, iain, 1
	   staOK = .true.
	end if
10	continue
	rewind(70)
...
c------------------------------------------------------------------
	...
	logical stnexist
        ...
        fname = evstr(1:length(evstr))//staname//'.sac'
c	write(6,*) fname
        open(50, file = fname,
     1           status = 'old',
     2           form = 'unformatted',
