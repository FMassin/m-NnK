...
if size(XYXred,2)==3 & size(XYXgreen,2)==3 %& numel(XYZyellow)==numel(XYXgreen) & numel(XYZyellow)==numel(XYXred)
    for i=1:size(XYZyellow,1)
        rmsred(i)= (sum(([ XYZyellow(i,1)-XYXred(:,1) ; XYZyellow(i,2)-XYXred(:,2)  ; (XYZyellow(i,3)-XYXred(:,3))./110  ]).^2)).^0.5 ;
        rmsgreen(i)=(sum(([ XYZyellow(i,1)-XYXgreen(:,1);  XYZyellow(i,2)-XYXgreen(:,2) ;  (XYZyellow(i,3)-XYXgreen(:,3))./110  ]).^2)).^0.5;
    end
