        ...
        for i3=dim3
            for i2=dim2
                if cases == 1 | (cases == 2 & d1 > 500)
                    ...
                    % progress bar update %%%%%%%%%%%%%%%%%
                    message=['Correlating phase ' num2str(i5) '/' num2str(d5) ' compo ' num2str(i4) '/' num2str(d4) ' station ' num2str(i3) '/' num2str(d3) ' on ' num2str(d1) ' events '];
                    clc;
                        ...
                        if cnt0 > 1
                            ...
                            for j=dim1
                                ...
                                if exist('Strdataless','var')==1;if numel(Strdataless{j,i2,i3,i4,i5})>=7;
                                        ...
                                        ccmax = CCCtmp(j,[1:j-1 j+1:size(CCCtmp,1)]) ; ccmax = ((max(ccmax(isnan(ccmax)==0)))) ; 
                                        %ccmax=1-(sum(abs(X(j,1:50)))/sum(abs(X(j,51:100))));
                                        Strdataless{j,i2,i3,i4,i5}{19}=ccmax;
                            ...
                            if exist('Strdataless','var')==1;Strdataless{max(dim1)+1,i2,i3,i4,i5}=Strdataless{master,i2,i3,i4,i5};end
                            ...
                            if strcmp(phases(i5),'P')==1 & numel(indend)==1
                                ...
                                for j=dim1;
                                    ...
                                    if exist('Strdataless','var')==1;if length(Strdataless{j,i2,i3,i4,indend})>7;
                                            Strdataless{j,i2,i3,i4,indend}{19}=(sum(logical(abs(Xall(:,55))>0))/size(Xall,2));
                                ...
                                Strwfs{max(dim1)+1,i2,i3,i4,indend}=sum(Xall)'./repmat(sum(logical(abs(Xall(:,55))>0)),size(Xall,2),1);
                                if exist('Strdataless','var')==1;Strdataless{max(dim1)+1,i2,i3,i4,indend}=Strdataless{master,i2,i3,i4,indend};end
